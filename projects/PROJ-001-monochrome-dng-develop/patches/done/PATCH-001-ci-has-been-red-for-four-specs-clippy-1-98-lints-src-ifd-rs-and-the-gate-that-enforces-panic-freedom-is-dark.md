...
task:
  ...
  complexity_actual: M             # stamped at ship — TWO defects, not one, and a punch-list round
...
cost:
  sessions:
    - cycle: patch
      ...
      tokens_total: 36375561
      estimated_usd: 75.58
      duration_minutes: 62
      recorded_at: 2026-08-22
      notes: "⚠ HOW THIS NUMBER WAS BOUNDED, because a main-loop cycle has no natural meter and `just cost-audit` rightly refuses null on a patch. The orchestrator session spans far more than PATCH-001, so reporting the session total would overstate this patch by a large multiple. Instead it is bounded BY TIMESTAMP: summed over the session's own transcript from 2026-08-22T20:14:04Z — the first PATCH-001 action, the `as_chunks` MSRV probe — to the end. That is an OBSERVABLE split, unlike a per-spec division of orchestration, which AGENTS.md correctly says cannot be observed and must not be invented. DEDUPED BY message.id and I say so: 72 distinct ids, 98.3% cache-read. estimated_usd computed PER-COMPONENT at published Opus rates on message.model = claude-opus-5 read from the transcript, NOT from tier_map (which is 1 for 4). It is an UPPER bound for the patch and a floor for the session: everything after the boundary is counted, including the two CI waits and the punch-list round.\n\nOrchestrator wrote both fixes and the record; that is why DEC-003's independent verify was kept, and it returned two ship-blockers on this author's own work. Orchestrator wrote both fixes and the record; that is why DEC-003's independent verify was kept, and it returned two ship-blockers on this author's own work. Punch-list round after verify: cleared SB-1 (guarded EVERY stage of the pipeline, not just the leading grep, and red-proofed it with a genuine leading-grep zero-match — the exact test the first attempt lacked) and SB-2 (corrected the inverted causality in four documents and SPLIT the conflated signal, because the project close would otherwise have fixed the toolchain and left the mute gate). Also caught in passing: `just new-spec` minted a duplicate SPEC-010 against a spec that exists only on an unmerged branch — renamed by hand, filed as spec-ids-collide-across-unmerged-branches."
    - cycle: verify
      agent: claude-opus-5
      interface: claude-code
  ...
  totals:
    tokens_total: 46375561
    estimated_usd: 100.48
    session_count: 2
---
