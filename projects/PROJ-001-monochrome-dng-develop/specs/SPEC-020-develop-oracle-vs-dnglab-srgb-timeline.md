...
- [ ] **build** — handoff pending; the design pass authorises the
  `ssimulacra2` dev-dep + its DEC to land in the same build pass
  (AGENTS.md §13 point 4), with the licence re-confirmed on the
  pinned tree before the DEC is written. Fixture choice is
