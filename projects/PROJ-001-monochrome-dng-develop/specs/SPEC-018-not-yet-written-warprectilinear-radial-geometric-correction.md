...
# SPEC-018: (not yet written) WarpRectilinear radial geometric correction

> **OUTLINE — `cycle: frame`.** This spec exists so its ID is stable and
> siblings can declare `depends_on: [SPEC-018]`. Capture **scope** (Context /
...
- The parent `STAGE-003` and this spec's place in its backlog
